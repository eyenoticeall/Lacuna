...
```bash
python -m pip install ./lacuna-0.9.0-*.whl ./lacuna_options-0.1.3-py3-none-any.whl
```
...
Version `0.1.3` preserves the exact `0.1.x` Python surface while expanding its compatible core range
to include additive Lacuna `0.9.x` releases. Validated chain evidence is recognized by the required
