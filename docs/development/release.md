...
versions, inspects both wheels and source archives, and writes one `SHA256SUMS`. For a `v0.7.0` tag
with extension `0.1.1`, the release set is exactly four core wheels, one extension wheel, two source
...
Python package metadata normalizes a candidate such as `0.7.0-rc.1` to `0.7.0rc1`; stable package
metadata is `0.7.0`. The release verifier owns this mapping and prevents the two surfaces from
drifting.
