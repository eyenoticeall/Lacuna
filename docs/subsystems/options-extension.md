...
0.6 milestone. It is released beside core but is not a core dependency.
...
version:      0.1.x (independent of Lacuna core 0.6.x)
```
...
python -m pip install \
  ./lacuna-0.6.0-cp311-abi3-<platform>.whl \
  ./lacuna_options-0.1.0-py3-none-any.whl
```

The initial extension supports `lacuna>=0.5,<0.7`. Its `0.1.x` exports and signatures are frozen in
