...
0.6 milestone. Patch `0.1.5` expands compatibility through additive Lacuna core `0.11.x`; it is
...
version:      0.1.x (independent of Lacuna core 0.11.x)
```
...
python -m pip install \
  ./lacuna-0.11.0-cp311-abi3-<platform>.whl \
  ./lacuna_options-0.1.5-py3-none-any.whl
```
...
`0.1.5` changes only dependency metadata and integration evidence: core `0.11` is additive, the
