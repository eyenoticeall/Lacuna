...
CONTRACT_PATH = Path(__file__).parents[1] / "fixtures" / "public-api-v0.12.json"
...
def test_root_exports_preserve_the_v0_12_contract() -> None:
    expected = _contract()["root_exports"]
    assert isinstance(expected, list)
    assert set(expected).issubset(lacuna.__all__)


def test_public_module_exports_preserve_the_v0_12_contract() -> None:
...
def test_public_call_signatures_match_the_v0_12_contract() -> None:
...
def test_v0_12_contract_declares_its_compatibility_chain() -> None:
    assert _contract()["package_series"] == "0.12"
    assert _contract()["inherited_contracts"] == [
        ...
        "lacuna-public-api-v0.9",
        "lacuna-public-api-v0.10",
        "lacuna-public-api-v0.11",
    ]
