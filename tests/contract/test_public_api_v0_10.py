...
CONTRACT_PATH = Path(__file__).parents[1] / "fixtures" / "public-api-v0.10.json"
...
def test_root_exports_preserve_the_v0_10_contract() -> None:
    expected = _contract()["root_exports"]
    assert isinstance(expected, list)
    assert set(expected).issubset(lacuna.__all__)


def test_public_module_exports_preserve_the_v0_10_contract() -> None:
...
def test_public_call_signatures_match_the_v0_10_contract() -> None:
...
def test_v0_10_contract_declares_its_compatibility_chain() -> None:
    assert _contract()["package_series"] == "0.10"
